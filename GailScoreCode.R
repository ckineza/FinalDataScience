  ...
  
{
  ...
  matrix.data = matrix(0,nrow = length(ages), ncol = 6)
  T_small = ages + 5
  T_small = ifelse(T_small <= 90, T_small,90)
  ID = seq(1,length(ages),1)
  N_Biop = rep(99,length(ages))
  HypPlas = rep(99,length(ages))
  Agemen = rep(99, length(ages))
  Age1st = rep(99,length(ages))
  N_Rels = rep(99,length(ages))
  T_90 = rep(90,length(ages))
...
#colnames(age1) <- c("Absolute risk(5 years)","Average absolute risk(5 years)", "Lifetime absolute risk","Average lifetime risk","age","firstname")
#select non duplicated names
  ...
  
{
  ...
  T_small = ages + 5
  T_small = ifelse(T_small <= 90, T_small,90)
  N_Biop = rep(99,length(race_changing))
  HypPlas = rep(99,length(race_changing))
  Agemen = rep(99, length(race_changing))
  Age1st = rep(99,length(race_changing))
  N_Rels = rep(99,length(race_changing))
  T_90 = rep(90,length(race_changing))
...
for(i in 1: nrow(data.complet))
{
  ...
  HypPlas <- rep(99, nrow(age.race.varation))
  AgeMen <- rep(99, nrow(age.race.varation))
  Age1st <- rep(99, length(age.race.varation))
  N_Rels <- rep(99, nrow(age.race.varation))
  m <- matrix(0, nrow = nrow(age.race.varation), ncol = 6)
  ID <- seq(1, nrow(age.race.varation), 1)
  T2 <- T1 + 5 # 5 years
  T2 <- ifelse(T2_5 <= 90, T2_5, 90) #plafond
